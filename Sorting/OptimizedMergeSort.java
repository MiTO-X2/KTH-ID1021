...
public class OptimizedMergeSort {
    ...
    // Recursive sort method with optimization
    ...
    private static void merge(int[] org, int[] aux, int lo, int mid, int hi) {
        ...
        System.arraycopy(org, lo, aux, lo, hi - lo + 1);
        ...
        for (int k = lo; k <= hi; k++) {
            if (i > mid) {
                org[k] = aux[j++];  // Left half exhausted
            } else if (j > hi) {
                org[k] = aux[i++];  // Right half exhausted
            } else if (aux[i] <= aux[j]) {
    ...
    public static void main(String[] args) {
        ...
        for (int size : sizes) {
            ...
            sort(array);  // Call optimized merge sort
